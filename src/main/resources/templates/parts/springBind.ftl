 ...
 *    (such as class or CSS styles or size)
 -->
<#macro formInput path classInput="" placeholder="" attributes="" fieldType="text" >
    <@bind path/>
    <input type="${fieldType}" placeholder="${placeholder}" class="${classInput}" id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" value="<#if fieldType!="password">${stringStatusValue}</#if>" ${attributes?no_esc}<@closeTag/>
</#macro>
